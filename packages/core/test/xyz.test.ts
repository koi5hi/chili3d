// Part of the Chili3d Project, under the AGPL-3.0 License.
// See LICENSE file in the project root for full license information.

import { getVectorComponent, Precision, XY, XYZ } from "../src";

describe("XYZ class tests", () => {
    describe("Basic properties and constructor", () => {
        test("should create XYZ instance with given values", () => {
            const point = new XYZ({ x: 1, y: 2, z: 3 });
            expect(point.x).toBe(1);
            expect(point.y).toBe(2);
            expect(point.z).toBe(3);

            expect(XYZ.fromArray(undefined as any)).toEqual(XYZ.zero);
            expect(XYZ.fromArray([1, 2, 3])).toEqual(point);
            expect(XYZ.fromArray([1, 2])).toEqual(new XYZ({ x: 1, y: 2, z: 0 }));
            expect(XYZ.fromArray([1, 2, 3, 4])).toEqual(new XYZ({ x: 1, y: 2, z: 3 }));
            // Single element array triggers ?? 0 for y and z
            expect(XYZ.fromArray([5])).toEqual(new XYZ({ x: 5, y: 0, z: 0 }));
        });

        test("should throw on NaN values", () => {
            expect(() => new XYZ({ x: NaN, y: 0, z: 0 })).toThrow("NaN in XYZ");
            expect(() => new XYZ({ x: 0, y: NaN, z: 0 })).toThrow("NaN in XYZ");
            expect(() => new XYZ({ x: 0, y: 0, z: NaN })).toThrow("NaN in XYZ");
        });

        test("should have correct static properties", () => {
            expect(XYZ.zero).toEqual(new XYZ({ x: 0, y: 0, z: 0 }));
            expect(XYZ.unitX).toEqual(new XYZ({ x: 1, y: 0, z: 0 }));
            expect(XYZ.unitY).toEqual(new XYZ({ x: 0, y: 1, z: 0 }));
            expect(XYZ.unitZ).toEqual(new XYZ({ x: 0, y: 0, z: 1 }));
            expect(XYZ.unitNX).toEqual(new XYZ({ x: -1, y: 0, z: 0 }));
            expect(XYZ.unitNY).toEqual(new XYZ({ x: 0, y: -1, z: 0 }));
            expect(XYZ.unitNZ).toEqual(new XYZ({ x: 0, y: 0, z: -1 }));
            expect(XYZ.one).toEqual(new XYZ({ x: 1, y: 1, z: 1 }));
        });

        test("should convert to string and array correctly", () => {
            const point = new XYZ({ x: 1, y: 2, z: 3 });
            expect(point.toString()).toBe("1, 2, 3");
            expect(point.toArray()).toEqual([1, 2, 3]);
        });
    });

    describe("Arithmetic operations", () => {
        test("should add vectors correctly", () => {
            const a = new XYZ({ x: 1, y: 2, z: 3 });
            const b = new XYZ({ x: 4, y: 5, z: 6 });
            const result = a.add(b);
            expect(result).toEqual(new XYZ({ x: 5, y: 7, z: 9 }));
        });

        test("should subtract vectors correctly", () => {
            const a = new XYZ({ x: 5, y: 7, z: 9 });
            const b = new XYZ({ x: 1, y: 2, z: 3 });
            const result = a.sub(b);
            expect(result).toEqual(new XYZ({ x: 4, y: 5, z: 6 }));
        });

        test("should multiply by scalar correctly", () => {
            const point = new XYZ({ x: 1, y: 2, z: 3 });
            const result = point.multiply(3);
            expect(result).toEqual(new XYZ({ x: 3, y: 6, z: 9 }));
        });

        test("should divide by scalar correctly", () => {
            const point = new XYZ({ x: 10, y: 20, z: 30 });
            const result = point.divided(2);
            expect(result).toEqual(new XYZ({ x: 5, y: 10, z: 15 }));
        });

        test("should return undefined when dividing by zero", () => {
            const point = new XYZ({ x: 10, y: 20, z: 30 });
            const result = point.divided(0);
            expect(result).toBeUndefined();

            const result2 = point.divided(Precision.Float / 2); // very small number
            expect(result2).toBeUndefined();
        });

        test("should reverse the vector correctly", () => {
            const point = new XYZ({ x: 1, y: 2, z: 3 });
            const reversed = point.reverse();
            expect(reversed).toEqual(new XYZ({ x: -1, y: -2, z: -3 }));

            const zero = XYZ.zero.reverse();
            expect(zero).toEqual(new XYZ({ x: 0, y: 0, z: 0 }));
        });
    });

    describe("Mathematical operations", () => {
        test("should calculate cross product correctly", () => {
            const a = new XYZ({ x: 1, y: 0, z: 0 });
            const b = new XYZ({ x: 0, y: 1, z: 0 });
            const result = a.cross(b);
            expect(result).toEqual(new XYZ({ x: 0, y: 0, z: 1 }));

            const c = new XYZ({ x: 2, y: 3, z: 4 });
            const d = new XYZ({ x: 5, y: 6, z: 7 });
            const result2 = c.cross(d);
            expect(result2).toEqual(new XYZ({ x: -3, y: 6, z: -3 })); // (3*7-4*6, 4*5-2*7, 2*6-3*5)
        });

        test("should calculate dot product correctly", () => {
            const a = new XYZ({ x: 1, y: 2, z: 3 });
            const b = new XYZ({ x: 4, y: 5, z: 6 });
            expect(a.dot(b)).toBe(32); // 1*4 + 2*5 + 3*6 = 4 + 10 + 18 = 32
        });

        test("should calculate length and length squared correctly", () => {
            const point = new XYZ({ x: 3, y: 4, z: 5 });
            expect(point.lengthSq()).toBe(50); // 9 + 16 + 25
            expect(point.length()).toBe(Math.sqrt(50));
        });

        test("should normalize vector correctly", () => {
            const point = new XYZ({ x: 3, y: 0, z: 0 });
            const normalized = point.normalize();
            expect(normalized).toEqual(new XYZ({ x: 1, y: 0, z: 0 }));

            const unit = new XYZ({ x: 1, y: 1, z: 1 });
            const normalized2 = unit.normalize();
            const length = Math.sqrt(3);
            expect(normalized2).toEqual(new XYZ({ x: 1 / length, y: 1 / length, z: 1 / length }));
        });

        test("should return undefined for zero vector normalization", () => {
            const normalized = XYZ.zero.normalize();
            expect(normalized).toBeUndefined();
        });
    });

    describe("Distance and geometric operations", () => {
        test("should calculate distance correctly", () => {
            const a = new XYZ({ x: 0, y: 0, z: 0 });
            const b = new XYZ({ x: 3, y: 4, z: 0 });
            expect(a.distanceTo(b)).toBe(5); // 3-4-5 triangle

            const c = new XYZ({ x: 1, y: 1, z: 1 });
            const d = new XYZ({ x: 4, y: 5, z: 1 });
            expect(c.distanceTo(d)).toBe(5); // 3-4-5 triangle in xy plane
        });

        test("should calculate center correctly", () => {
            const p1 = new XYZ({ x: 0, y: 0, z: 0 });
            const p2 = new XYZ({ x: 4, y: 6, z: 8 });
            const center = XYZ.center(p1, p2);
            expect(center).toEqual(new XYZ({ x: 2, y: 3, z: 4 }));
        });
    });

    describe("Angle operations", () => {
        test("should calculate angle correctly", () => {
            const a = new XYZ({ x: 10, y: 0, z: 0 });
            const b = new XYZ({ x: 0, y: 10, z: 0 });
            expect(a.angleTo(b)).toBe(Math.PI / 2);
            expect(a.angleTo(new XYZ({ x: 10, y: 0, z: 0 }))).toBe(0);
            expect(a.angleTo(new XYZ({ x: 10, y: 10, z: 0 }))).toBe(Math.PI / 4);
            expect(a.angleTo(new XYZ({ x: 0, y: 10, z: 0 }))).toBe(Math.PI / 2);
            expect(a.angleTo(new XYZ({ x: -10, y: 10, z: 0 }))).toBe((Math.PI * 3) / 4);
            expect(a.angleTo(new XYZ({ x: -10, y: 0, z: 0 }))).toBe(Math.PI);
            expect(a.angleTo(new XYZ({ x: -10, y: -10, z: 0 }))).toBe((Math.PI * 3) / 4);
            expect(a.angleTo(new XYZ({ x: 10, y: -10, z: 0 }))).toBe(Math.PI / 4);
        });

        test("should return undefined for zero vector angle", () => {
            expect(XYZ.zero.angleTo(new XYZ({ x: 0, y: 0, z: 0 }))).toBe(undefined);
            expect(XYZ.zero.angleTo(new XYZ({ x: 1, y: 0, z: 0 }))).toBeUndefined();
            expect(new XYZ({ x: 1, y: 0, z: 0 }).angleTo(XYZ.zero)).toBeUndefined();
        });

        test("should calculate angle on plane correctly", () => {
            const a = new XYZ({ x: 10, y: 0, z: 0 });
            const b = new XYZ({ x: 0, y: 10, z: 0 });
            expect(a.angleOnPlaneTo(b, XYZ.unitZ)).toBe(Math.PI / 2);
            expect(a.angleOnPlaneTo(new XYZ({ x: 10, y: 0, z: 0 }), XYZ.unitZ)).toBe(0);
            expect(a.angleOnPlaneTo(new XYZ({ x: 10, y: 10, z: 0 }), XYZ.unitZ)).toBe(Math.PI / 4);
            expect(a.angleOnPlaneTo(new XYZ({ x: 0, y: 10, z: 0 }), XYZ.unitZ)).toBe(Math.PI / 2);
            expect(a.angleOnPlaneTo(new XYZ({ x: -10, y: 10, z: 0 }), XYZ.unitZ)).toBe((Math.PI * 3) / 4);
            expect(a.angleOnPlaneTo(new XYZ({ x: -10, y: 0, z: 0 }), XYZ.unitZ)).toBe(Math.PI);
            expect(a.angleOnPlaneTo(new XYZ({ x: -10, y: -10, z: 0 }), XYZ.unitZ)).toBe((Math.PI * 5) / 4);
            expect(a.angleOnPlaneTo(new XYZ({ x: 10, y: -10, z: 0 }), XYZ.unitZ)).toBe((Math.PI * 7) / 4);

            expect(new XYZ({ x: 10, y: 10, z: 0 }).angleOnPlaneTo(a, XYZ.unitZ)).toBe((Math.PI * 7) / 4);
            expect(a.angleOnPlaneTo(new XYZ({ x: 10, y: 10, z: 0 }), new XYZ({ x: 0, y: 0, z: -1 }))).toBe(
                (Math.PI * 7) / 4,
            );
        });

        test("should return undefined for zero normal in angleOnPlaneTo", () => {
            const a = new XYZ({ x: 1, y: 0, z: 0 });
            const b = new XYZ({ x: 0, y: 1, z: 0 });
            expect(a.angleOnPlaneTo(b, XYZ.zero)).toBeUndefined();
            expect(XYZ.zero.angleOnPlaneTo(XYZ.zero, XYZ.unitZ)).toBe(undefined);
        });
    });

    describe("Rotation operations", () => {
        test("should rotate vector correctly", () => {
            const v = XYZ.unitX.add(XYZ.unitZ);
            expect(v.rotate(v, 90)?.isEqualTo(v)).toBe(true);
            expect(
                XYZ.unitX
                    .rotate(XYZ.unitZ, Math.PI / 4)
                    ?.isEqualTo(new XYZ({ x: 1, y: 1, z: 0 }).normalize()!),
            ).toBe(true);
            expect(XYZ.unitX.rotate(XYZ.unitZ, Math.PI / 2)?.isEqualTo(new XYZ({ x: 0, y: 1, z: 0 }))).toBe(
                true,
            );
            expect(XYZ.unitX.rotate(XYZ.unitZ, Math.PI / 1)?.isEqualTo(new XYZ({ x: -1, y: 0, z: 0 }))).toBe(
                true,
            );
            expect(
                XYZ.unitX.rotate(XYZ.unitZ, Math.PI * 1.5)?.isEqualTo(new XYZ({ x: 0, y: -1, z: 0 })),
            ).toBe(true);

            const result = XYZ.unitX.rotate(XYZ.unitZ, Math.PI / 2);
            expect(result?.isEqualTo(XYZ.unitY, 1e-10)).toBe(true);

            const result2 = XYZ.unitY.rotate(XYZ.unitZ, Math.PI / 2);
            expect(result2?.isEqualTo(XYZ.unitX.reverse(), 1e-10)).toBe(true);
        });

        test("should handle edge cases for rotation", () => {
            const result = XYZ.unitX.rotate(XYZ.zero, Math.PI / 2);
            expect(result).toBeUndefined();
        });
    });

    describe("Comparison methods", () => {
        test("isEqualTo should work correctly", () => {
            const a = new XYZ({ x: 1, y: 2, z: 3 });
            const b = new XYZ({ x: 1, y: 2, z: 3 });
            expect(a.isEqualTo(b)).toBe(true);

            const c = new XYZ({ x: 1.1, y: 2, z: 3 });
            expect(a.isEqualTo(c, 0.2)).toBe(true); // within tolerance
            expect(a.isEqualTo(c, 0.05)).toBe(false); // outside tolerance
        });

        test("isPerpendicularTo should work correctly", () => {
            const a = new XYZ({ x: 1, y: 0, z: 0 });
            const b = new XYZ({ x: 0, y: 1, z: 0 });
            expect(a.isPerpendicularTo(b)).toBe(true);
            expect(a.isPerpendicularTo(b.reverse())).toBe(true);

            const c = new XYZ({ x: 1, y: 1, z: 0 });
            expect(a.isPerpendicularTo(c, 0.1)).toBe(false);
        });

        test("isParallelTo should work correctly", () => {
            const a = new XYZ({ x: 1, y: 0, z: 0 });
            const b = new XYZ({ x: 2, y: 0, z: 0 });
            expect(a.isParallelTo(b)).toBe(true);

            const c = new XYZ({ x: -1, y: 0, z: 0 });
            expect(a.isParallelTo(c)).toBe(true); // opposite direction is still parallel
        });

        test("isOppositeTo should work correctly", () => {
            const a = new XYZ({ x: 1, y: 0, z: 0 });
            const b = new XYZ({ x: -1, y: 0, z: 0 });
            expect(a.isOppositeTo(b)).toBe(true);

            const c = new XYZ({ x: 0, y: 1, z: 0 });
            expect(a.isOppositeTo(c)).toBe(false);
        });

        test("isPerpendicularTo returns false for zero vector", () => {
            expect(XYZ.zero.isPerpendicularTo(XYZ.unitX)).toBe(false);
            expect(XYZ.unitX.isPerpendicularTo(XYZ.zero)).toBe(false);
        });

        test("isParallelTo returns false for zero vector", () => {
            expect(XYZ.zero.isParallelTo(XYZ.unitX)).toBe(false);
            expect(XYZ.unitX.isParallelTo(XYZ.zero)).toBe(false);
        });

        test("isOppositeTo returns false for zero vector", () => {
            expect(XYZ.zero.isOppositeTo(XYZ.unitX)).toBe(false);
            expect(XYZ.unitX.isOppositeTo(XYZ.zero)).toBe(false);
        });
    });

    describe("XY class tests", () => {
        test("should calculate angle for XY vectors", () => {
            const v1 = XY.unitX;
            const v2 = XY.unitY;
            expect(v1.angleTo(v2)).toBe(Math.PI / 2);
        });
    });
});

describe("getVectorComponent", () => {
    test("should return the x component when index is 0", () => {
        const point = { x: 1, y: 2, z: 3 };
        expect(getVectorComponent(point, 0)).toBe(1);
    });

    test("should return the y component when index is 1", () => {
        const point = { x: 1, y: 2, z: 3 };
        expect(getVectorComponent(point, 1)).toBe(2);
    });

    test("should return the z component when index is 2", () => {
        const point = { x: 1, y: 2, z: 3 };
        expect(getVectorComponent(point, 2)).toBe(3);
    });

    test("should throw an error when index is out of range", () => {
        const point = { x: 1, y: 2, z: 3 };
        expect(() => getVectorComponent(point, 3)).toThrow("index out of range");
        expect(() => getVectorComponent(point, -1)).toThrow("index out of range");
    });
});
